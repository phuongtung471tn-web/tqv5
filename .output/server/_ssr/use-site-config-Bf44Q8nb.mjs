...
async function saveLead(lead, config) {
  ...
  if (isBrowser()) {
    ...
    window.dispatchEvent(
      new CustomEvent(LEAD_CREATED_EVENT, { detail: record }),
    );
  }
  return record;
}
...
function loadAnalytics() {
  ...
  try {
    return normalizeAnalytics(
      JSON.parse(window.localStorage.getItem(ANALYTICS_KEY) || "{}"),
    );
  } catch {
...
function saveAnalytics(state) {
  ...
  window.dispatchEvent(
    new CustomEvent(ANALYTICS_UPDATED_EVENT, { detail: state }),
  );
}
...
function clearAnalytics() {
  ...
  window.dispatchEvent(
    new CustomEvent(ANALYTICS_UPDATED_EVENT, { detail: emptyAnalytics() }),
  );
}
...
export {
  clearAnalytics as a,
  isDuplicateLead as c,
  saveLead as d,
  testSupabaseConnection as f,
  useSiteConfig as h,
  SiteConfigProvider as i,
  loadAnalytics as l,
  trackVisit as m,
  DEFAULT_CONFIG as n,
  clearLeads as o,
  trackConversion as p,
  LEAD_CREATED_EVENT as r,
  exportLeadsCsv as s,
  ANALYTICS_UPDATED_EVENT as t,
  loadLeads as u,
};
