var e = Object.create,
          ...
          case `object`:
            switch (e.$$typeof) {
              ...
              case n:
                ...
                break;
              case d:
                return ((c = e._init), P(c(e._payload), r, i, a, o));
            }
        }
      if (c)
        return (
  ...
  m = {
    ...
    landing: {
      sectionsArray: [
        { id: `hero`, type: `hero`, label: `Hero`, enabled: !0, order: 0 },
        { id: `stats`, type: `stats`, label: `Stats`, enabled: !0, order: 1 },
        {
          id: `pains`,
          type: `pains`,
          label: `Pain points`,
        ...
        { id: `faq`, type: `faq`, label: `FAQ`, enabled: !0, order: 9 },
        {
          id: `finalCta`,
          type: `finalCta`,
          label: `CTA cuối`,
...
Trân trọng.`,
  ...
  h = `funnel_site_config_v1`,
  g = `funnel_leads_v1`,
  _ = `funnel_analytics_v1`,
  v = `funnel_backup_snapshots_v1`,
  y = `funnel:lead-created`,
  b = `funnel:analytics-updated`,
  x = `funnel_configs`;
function S(e, t) {
  if (!t) return structuredClone(e);
  let n = structuredClone(t);
  n.landing?.sections &&
    !n.landing.sectionsArray &&
    ((n.landing.sectionsArray = n.landing.sections.map((e, t) => ({
      ...e,
      type: e.type || e.id,
      order: t,
    }))),
    delete n.landing.sections);
  let r = (e, t) => {
    if (
      t &&
      typeof t == `object` &&
      ...
      let n = { ...e };
      for (let [e, i] of Object.entries(t)) n[e] = r(n[e], i);
      return n;
    }
    return t === void 0 ? e : t;
  };
  return r(structuredClone(e), n);
}
function C() {
  return typeof window < `u`;
}
function w() {
  if (!C()) return structuredClone(m);
  try {
    let e = window.localStorage.getItem(h);
    return S(m, e ? JSON.parse(e) : null);
  } catch {
...
async function T(e) {
  if (
    !C() ||
    e.admin.storageMode !== `database` ||
    !e.admin.supabaseUrl ||
    !e.admin.supabaseAnonKey
  )
    return null;
  try {
    let t = await fetch(
      `${e.admin.supabaseUrl.replace(/\/$/, ``)}/rest/v1/${x}?id=eq.1&select=data`,
      {
        headers: {
          apikey: e.admin.supabaseAnonKey,
          Authorization: `Bearer ${e.admin.supabaseAnonKey}`,
        },
      },
    );
    if (!t.ok) return null;
    let n = await t.json();
    return n[0]?.data ? S(m, n[0].data) : null;
  } catch {
    return null;
  }
}
function E(e) {
  if (C()) {
    ...
    try {
      let t = JSON.parse(window.localStorage.getItem(v) || `[]`);
      (t.unshift({ at: new Date().toISOString(), config: e }),
        window.localStorage.setItem(v, JSON.stringify(t.slice(0, 10))));
    } catch {}
    e.admin.storageMode === `database` &&
      e.admin.supabaseUrl &&
      e.admin.supabaseAnonKey &&
...
function D() {
  return (C() && window.localStorage.removeItem(h), structuredClone(m));
}
function O(e) {
  if (!C()) return;
  let t = `// AUTO-GENERATED — dán đè vào src/config/site-config.ts (phần DEFAULT_CONFIG)\nexport const DEFAULT_CONFIG = ${JSON.stringify(e, null, 2)};\n`,
    n = new Blob([t], { type: `text/javascript` }),
    r = URL.createObjectURL(n),
    i = document.createElement(`a`);
  ((i.href = r),
    (i.download = `site-config.export.js`),
    i.click(),
...
function k() {
  if (!C()) return [];
  try {
    return JSON.parse(window.localStorage.getItem(g) || `[]`);
  } catch {
    return [];
  }
}
function A(e) {
  ...
  return k().some((n) => n.phone === e && new Date(n.at).getTime() > t);
}
function j() {
  C() && window.localStorage.removeItem(g);
}
async function M(e, t) {
  let n =
      t?.admin.storageMode === `database` &&
      t.admin.supabaseUrl &&
      t.admin.supabaseAnonKey
        ? `database`
        : `local`,
    r = { ...e, storage: n };
  if (
    (n === `database` &&
      t &&
      ((await N(r, t.admin.supabaseUrl, t.admin.supabaseAnonKey)) ||
        (r.storage = `local`)),
    C())
  ) {
    let e = k();
    (e.unshift(r),
      window.localStorage.setItem(g, JSON.stringify(e.slice(0, 500))),
      window.dispatchEvent(new CustomEvent(y, { detail: r })));
  }
  return r;
}
async function N(e, t, n) {
  try {
    return (
      await fetch(`${t.replace(/\/$/, ``)}/rest/v1/leads`, {
        method: `POST`,
        headers: {
        ...
        body: JSON.stringify([
          {
            ...
            city: e.city ?? null,
            major: e.major ?? null,
            ai_score: e.aiScore ?? null,
            ai_rank: e.aiRank ?? null,
...
function P(e) {
  ...
  let t = [
    ...
    n = e.map((e) =>
      t.map((t) => `"${String(e[t] ?? ``).replace(/"/g, `""`)}"`).join(`,`),
    ),
    r = [t.join(`,`), ...n].join(`
`),
    i = new Blob([`﻿` + r], { type: `text/csv;charset=utf-8` }),
    a = URL.createObjectURL(i),
    o = document.createElement(`a`);
  ((o.href = a),
    (o.download = `leads-${new Date().toISOString().slice(0, 10)}.csv`),
    o.click(),
...
function I(e) {
  return e.trim().slice(0, 100) || `direct`;
}
function L(e) {
  let t = F();
  ((t.visits = Number.isFinite(e?.visits) ? Math.max(0, Number(e?.visits)) : 0),
    (t.leads = Number.isFinite(e?.leads) ? Math.max(0, Number(e?.leads)) : 0));
  for (let [n, r] of Object.entries(e?.bySource || {}))
    Number.isFinite(r) && (t.bySource[I(n)] = Math.max(0, Number(r)));
  for (let [n, r] of Object.entries(e?.bySourceStats || {}))
    r &&
      (t.bySourceStats[I(n)] = {
        visits: Number.isFinite(r.visits) ? Math.max(0, Number(r.visits)) : 0,
        leads: Number.isFinite(r.leads) ? Math.max(0, Number(r.leads)) : 0,
      });
  for (let [e, n] of Object.entries(t.bySource))
    t.bySourceStats[e] = t.bySourceStats[e] || { visits: n, leads: 0 };
  for (let [n, r] of Object.entries(e?.byVariant || {}))
    r &&
      (t.byVariant[n] = {
        visits: Number.isFinite(r.visits) ? Math.max(0, Number(r.visits)) : 0,
        leads: Number.isFinite(r.leads) ? Math.max(0, Number(r.leads)) : 0,
...
function R() {
  if (!C()) return F();
  try {
    return L(JSON.parse(window.localStorage.getItem(_) || `{}`));
  } catch {
    return F();
  }
}
function z(e) {
  C() &&
    ...
    window.dispatchEvent(new CustomEvent(b, { detail: e })));
}
function B(e, t) {
  let n = R();
  n.visits += 1;
  let r = I(e);
  ((n.bySource[r] = (n.bySource[r] || 0) + 1),
    (n.bySourceStats[r] = n.bySourceStats[r] || { visits: 0, leads: 0 }),
    (n.bySourceStats[r].visits += 1),
    t &&
      ((n.byVariant[t] = n.byVariant[t] || { visits: 0, leads: 0 }),
      (n.byVariant[t].visits += 1)),
    z(n));
}
function V(e, t) {
  let n = R();
  n.leads += 1;
  let r = I(e);
  ((n.bySource[r] = n.bySource[r] || 0),
    (n.bySourceStats[r] = n.bySourceStats[r] || { visits: 0, leads: 0 }),
    (n.bySourceStats[r].leads += 1),
    t &&
      ((n.byVariant[t] = n.byVariant[t] || { visits: 0, leads: 0 }),
      (n.byVariant[t].leads += 1)),
    z(n));
}
function H() {
  C() &&
    (window.localStorage.removeItem(_),
    window.dispatchEvent(new CustomEvent(b, { detail: F() })));
}
async function U(e) {
  try {
    let { supabaseUrl: t, supabaseAnonKey: n } = e.admin;
    await fetch(`${t.replace(/\/$/, ``)}/rest/v1/${x}?on_conflict=id`, {
      method: `POST`,
      headers: {
        "Content-Type": `application/json`,
      ...
      body: JSON.stringify([
        { id: 1, data: e, updated_at: new Date().toISOString() },
      ]),
    });
  } catch (e) {
    console.log(`[v0] Supabase sync failed:`, e.message);
  }
}
async function W(e, t) {
  try {
    let n = await fetch(`${e.replace(/\/$/, ``)}/rest/v1/`, {
      headers: { apikey: t, Authorization: `Bearer ${t}` },
    });
    return n.ok || n.status === 404;
  } catch {
    return !1;
  }
}
...
function q({ children: e }) {
  ...
  let a = (0, p.useCallback)((e) => {
    ...
    c = (0, p.useCallback)(() => {
      (n((e) => ({ ...e, landing: structuredClone(m.landing) })), i(!0));
    }, []),
    l = (0, p.useCallback)(() => {
      n((e) => (O(e), e));
    }, []),
  ...
  return (0, G.jsx)(K.Provider, { value: u, children: e });
}
function J() {
  let e = (0, p.useContext)(K);
  if (!e) throw Error(`useSiteConfig must be used within SiteConfigProvider`);
  return e;
}
var Y = (...e) =>
    e
  ...
  ne = (e, t) => {
    let n = (0, p.forwardRef)(({ className: n, ...r }, i) =>
      (0, p.createElement)(te, {
        ref: i,
        iconNode: t,
        className: Y(`lucide-${X(Q(e))}`, `lucide-${e}`, n),
        ...r,
      }),
    );
