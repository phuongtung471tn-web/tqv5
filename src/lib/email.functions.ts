...
async function sendWithGmail(data: z.infer<typeof schema>) {
  ...
  if (!clientId || !clientSecret || !refreshToken) return { sent: false as const, reason: "missing_gmail_secrets" as const };
  ...
  if (!tokenResponse.ok) return { sent: false as const, reason: "gmail_token_error" as const };
  const token = (await tokenResponse.json()) as { access_token?: string };
  if (!token.access_token) return { sent: false as const, reason: "gmail_token_error" as const };

  const raw = [`From: ${data.from}`, `To: ${data.to}`, `Subject: ${data.subject}`, "Content-Type: text/plain; charset=UTF-8", "", data.text].join("\r\n");
  ...
  return response.ok ? { sent: true as const } : { sent: false as const, reason: "gmail_send_error" as const };
}
